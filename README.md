...
既に `[mcp_servers.ts-review-graph]` がある場合はエントリを重複させず、`args` の中の ts-review-graph の version 指定だけを差し替えます（`--log-level debug` のように独自に足した引数はそのまま残ります）。`command` を独自の値へ変えている場合はそれを保持し、`command` が無いときだけ `"npx"` を補います。`args` に ts-review-graph の package 指定が1つも無い場合（`command = "node"` などで独自に起動している場合）は、どう起動したいのかを推測できないため書き換えずに中止します — その場合は手動で version を更新してください。`env` は書かず、既存の `TS_REVIEW_GRAPH_DB`（`.mcp.json` から写したパスなど）があれば除去します — `env` の他のキーはそのまま残ります。他の `[mcp_servers.*]` エントリや他のセクションには触れません。
